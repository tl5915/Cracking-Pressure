...
void setup() {
  ...
  digitalWrite(buttonGroundPin, LOW);     // Digital ground for button
  pinMode(rbgPin, OUTPUT);
  digitalWrite(rbgPin, LOW);              // LED off
  delay(10);
  pinMode(resetButtonPin, INPUT_PULLUP);  // Button
...
void loop() {
  ...
  if (currentTime - lastSampleTime >= 25 && !plotDisplayed) {  // 40 Hz
    ...
    float pressure_inchH2O = pressure_kPa * 4.01865;                     // 1 kPa = 4.01865 inch H2O
    if (pressure_inchH2O > max_inchH2O) max_inchH2O = pressure_inchH2O;  // Record peak pressure
    recordedPressure[recordIndex] = pressure_inchH2O;                    // Record 200 samples
    ...
    int batteryPercentage = round(123 - (123 / pow((1 + pow((batteryVoltage / 3.7), 80)), 0.165)));  // LiPo battery percentage curve
    if (batteryPercentage > 100) batteryPercentage = 100;                                            // Maximum 100%
    ...
    display.print(F("3"));
    // Needle
    pressure_inchH2O = constrain(pressure_inchH2O, -1, 3);  // Gauge range -1 to 3 kPa
